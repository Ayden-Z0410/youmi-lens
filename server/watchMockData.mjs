/**
 * Youmi Watch — server-side mock fallback data.
 *
 * Returned by the read endpoints when the watch_* tables are empty (or a read
 * fails after the admin check passed), so the dashboard stays usable until the
 * write call sites are wired. Shapes mirror the frontend page data
 * (src/youmi-watch/data/mockData.ts) so the client can switch to live endpoints
 * with minimal changes.
 *
 * Contains NO secrets and NO real provider data — placeholder values only.
 */
...
export const MOCK_OVERVIEW = {
  metrics: [
    { id: 'active-users', label: 'Active Users', icon: 'users', value: '1,284', description: 'Daily active across Mac & iPad', trend: { direction: 'up', value: '12.4%', note: 'vs last week' } },
    { id: 'recordings', label: 'Recordings Today', icon: 'mic', value: '342', description: 'Captured & processed', trend: { direction: 'up', value: '8.1%', note: 'vs yesterday' } },
    { id: 'ai-minutes', label: 'AI Minutes', icon: 'sparkles', value: '8,640', description: 'Transcription + summaries', trend: { direction: 'up', value: '5.3%', note: 'vs last week' } },
    { id: 'error-rate', label: 'Error Rate', icon: 'alert', value: '0.4%', description: 'Failed jobs / total', trend: { direction: 'down', value: '0.2pp', note: 'improving' } },
  ],
  usageTrend: {
    labels: DAYS,
    yMax: 10000,
  ...
  alerts: [
    { id: 'a1', severity: 'warning', title: 'Supabase storage approaching limit', detail: 'Project storage at 82% of the current plan quota.', time: '14 min ago' },
    { id: 'a2', severity: 'success', title: 'Deepgram latency recovered', detail: 'Speech API p95 latency back under 150ms.', time: '1 hr ago' },
    { id: 'a3', severity: 'warning', title: 'Signup codes running low', detail: '38 unused beta invite codes remaining.', time: '3 hr ago' },
    { id: 'a4', severity: 'info', title: 'Nightly backup completed', detail: 'All recordings & transcripts snapshotted successfully.', time: '6 hr ago' },
  ],
  activity: [
...
export const MOCK_PROVIDERS = {
  metrics: [
    { id: 'connected', label: 'Connected', icon: 'link', value: '5', description: 'Deepgram, DashScope, +3', status: { kind: 'operational', label: 'Active' } },
    { id: 'healthy', label: 'Healthy', icon: 'check-circle', value: '4', description: 'Services operating normally', status: { kind: 'normal', label: 'Normal' } },
    { id: 'warnings', label: 'Warnings', icon: 'alert', value: '1', description: 'Supabase storage limit', status: { kind: 'watch', label: 'Watch' } },
    { id: 'offline', label: 'Offline', icon: 'offline', value: '0', description: 'No provider outages', status: { kind: 'stable', label: 'Stable' } },
  ],
  providers: [
    { id: 'deepgram', name: 'Deepgram', kind: 'Speech-to-Text API', icon: 'mic', status: 'normal', statusLabel: 'Normal', usage: '428.2 min', usageNote: 'this month', cost: '$42.18', health: '99.9%', healthNote: '2 min ago' },
    { id: 'dashscope', name: 'DashScope', kind: 'LLM / Summaries', icon: 'sparkles', status: 'normal', statusLabel: 'Normal', usage: '1.2M tok', usageNote: 'this month', cost: '$28.40', health: '99.7%', healthNote: '1 min ago' },
    { id: 'railway', name: 'Railway', kind: 'API Hosting', icon: 'server', status: 'normal', statusLabel: 'Normal', usage: '99.98%', usageNote: 'uptime', cost: '$20.00', health: '100%', healthNote: '30 sec ago' },
    { id: 'supabase', name: 'Supabase', kind: 'Database & Storage', icon: 'database', status: 'degraded', statusLabel: 'Degraded', usage: '82% disk', usageNote: 'storage used', cost: '$25.00', health: '97.2%', healthNote: '4 min ago' },
    { id: 'openai', name: 'OpenAI', kind: 'Fallback LLM', icon: 'sparkles', status: 'normal', statusLabel: 'Normal', usage: '210K tok', usageNote: 'this month', cost: '$11.62', health: '99.9%', healthNote: '3 min ago' },
  ],
  usageTrend: {
    labels: DAYS,
    yMax: 10000,
...
export const MOCK_ALERTS = {
  metrics: [
    { id: 'active-alerts', label: 'Active Alerts', icon: 'alert', value: '2', description: 'Require attention' },
    { id: 'resolved-today', label: 'Resolved Today', icon: 'check-circle', value: '5', description: 'Cleared automatically' },
    { id: 'critical', label: 'Critical', icon: 'alert', value: '1', description: 'High severity' },
    { id: 'total-week', label: 'Total This Week', icon: 'logs', value: '8', description: 'Across all providers' },
  ],
  rows: [
    { id: 'al-supabase-storage', severity: 'critical', title: 'Supabase storage above 75%', provider: 'Supabase', trigger: 'storage_used > 75%', time: '9m ago', status: 'active' },
    { id: 'al-dashscope-cost', severity: 'warning', title: 'DashScope daily cost exceeds threshold', provider: 'DashScope', trigger: 'daily_cost > $3', time: '24m ago', status: 'active' },
    { id: 'al-brevo-credits', severity: 'warning', title: 'Brevo credits below threshold', provider: 'Brevo', trigger: 'credits < 500', time: '1h ago', status: 'active' },
    { id: 'al-railway-deploy', severity: 'info', title: 'Railway deployment healthy', provider: 'Railway', trigger: 'deploy_status = success', time: '2h ago', status: 'resolved' },
  ],
  rules: [
  ...
  selectedDetail: {
    alertId: 'al-supabase-storage', provider: 'Supabase', trigger: 'Storage used above 75%',
    relatedMetric: '78% storage used', relatedPercent: 78,
    suggestedAction: 'Review storage bucket usage and remove unused lecture audio files.',
  },
}

export const MOCK_COSTS = {
  metrics: [
    { id: 'total-estimated', label: 'Total Estimated', icon: 'cost', value: '$1,842.35', description: 'This month' },
    { id: 'today', label: 'Today', icon: 'cost', value: '$62.40', description: 'Current daily spend' },
    { id: 'this-week', label: 'This Week', icon: 'cost', value: '$418.20', description: 'Rolling 7 days' },
    { id: 'forecast', label: 'Forecast', icon: 'trend', value: '$2,310', description: 'Projected month-end' },
  ],
  trend: {
    labels: DAYS,
    yMax: 24,
  ...
  budget: { monthlyBudget: '$2,500', currentSpend: '$1,842.35', remaining: '$657.65', usagePercent: 74, status: 'watch', statusLabel: 'Watch' },
  breakdown: [
  ...
  forecast: { projectedCost: '$2,310', budgetRemainingAfter: '$190', riskLevel: 'Medium', riskStatus: 'watch', suggestedAction: 'Watch DashScope daily token usage and Supabase storage growth.' },
}
...
export const MOCK_LOGS = {
  ...
  selectedDetail: {
    provider: 'Supabase', event: 'Storage usage checked', status: 'warning', statusLabel: 'Warning',
    requestId: 'req_sb_7740', relatedMetric: 'storage_used = 78%', relatedUser: 'system', recordingId: '—',
    retryCount: 0, message: 'Storage usage is approaching the configured warning threshold.',
