...
import { shouldCloseAuxiliaryWebviewWindow } from './lib/tauriWindowLabels'
...
/** Same-process safety: if anything created extra auth webviews, drop them after auth via deep link. */
async function tauriCloseNonMainWebviewWindows(): Promise<void> {
  ...
  try {
    ...
    await Promise.all(
      wins.filter((w) => shouldCloseAuxiliaryWebviewWindow(w.label)).map((w) => w.close()),
    )
  } catch (e) {
